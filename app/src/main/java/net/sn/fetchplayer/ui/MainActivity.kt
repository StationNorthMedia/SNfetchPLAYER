...
import net.sn.fetchplayer.databinding.ActivityMainBinding
import android.widget.ImageButton
import android.widget.LinearLayout
import net.sn.fetchplayer.data.ArtistLexiconRepository
...
class MainActivity : AppCompatActivity(), RadioService.ServiceListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        net.sn.fetchplayer.manager.QueenQuotesManager.init(this)

        val initialSyncState = net.sn.fetchplayer.manager.RemoteAssetSyncManager.checkLocalAssetsStatus(this)
    ...
    private fun setupArtistHistoryRecyclerView() {
        artistHistoryAdapter = ArtistHistoryAdapter(
            context = this,
            items = net.sn.fetchplayer.data.ArtistHistoryRepository.getHistory().toMutableList(),
            onItemClick = { track ->
    ...
    private fun showAppUpdateDialog(updateInfo: net.sn.fetchplayer.manager.AppUpdateInfo) {
        ...
        btnDownload.setOnClickListener {
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                val apkFile = net.sn.fetchplayer.manager.AppUpdateManager.downloadApk(
    ...
    private fun loadWikipediaArtistInfo(track: Track) {
        ...
        if (isStation) {
            val quote = net.sn.fetchplayer.manager.QueenQuotesManager.getRandomQuote()
            ...
            net.sn.fetchplayer.data.ArtistHistoryRepository.addOrUpdateItem(stationItem)
            artistHistoryAdapter.setItems(net.sn.fetchplayer.data.ArtistHistoryRepository.getHistory())
            binding.rvArtistHistory.scrollToPosition(0)
            updateRadioWikipediaPanel("👑 THE QUEEN", track.title, quoteText, null, quoteColor)
            return
        }

        val existingEntry = net.sn.fetchplayer.data.ArtistHistoryRepository.getHistory().firstOrNull { it.track.artist.equals(track.artist, ignoreCase = true) }
        ...
        lifecycleScope.launch(Dispatchers.Main) {
            ...
            net.sn.fetchplayer.data.ArtistHistoryRepository.addOrUpdateItem(historyItem)
            artistHistoryAdapter.setItems(net.sn.fetchplayer.data.ArtistHistoryRepository.getHistory())
            binding.rvArtistHistory.scrollToPosition(0)
